  ...
  {rowStarts.map((startMin, i) => {
    ...
    return (
      <tr key={startMin} className={`border-t ${stripe}`}>
        {/* עמודת השעה */}
        <td className="p-1 sm:p-2 text-center tabular-nums text-gray-600 align-middle whitespace-nowrap text-[9px] sm:text-xs h-14 sm:h-16">
          {hh}:{mm}
        </td>
          ...
          return (
            <td key={dow} className="p-1 sm:p-2 align-top">
              {slot ? (
                <button
                  onClick={() => toggle(slot)}
                  className={`relative block w-full rounded-xl border text-center transition h-14 sm:h-16
                    px-3 sm:px-4
                    ${slot.is_booked
                ...
                >
                  <div className="flex h-full flex-col items-center justify-center leading-tight gap-0.5">
                    {/* שם תלמיד – מודגש וגדול יותר */}
                    {slot.is_booked && (
                      <div className="text-[12px] sm:text-[14px] font-medium max-w-full truncate">
                ...
                <div className="h-14 sm:h-16 rounded-xl border border-dashed border-gray-200" />
              )}
            </td>
          );
        })}
